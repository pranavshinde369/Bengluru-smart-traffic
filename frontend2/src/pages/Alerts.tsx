...
const zones = ['Silk Board Junction', 'KR Market', 'MG Road', 'Hebbal', 'Whitefield'];
...
function generateChallan(): string {
  ...
  return `BTP-${dateStr}-${String(globalChallanIndex).padStart(4, '0')}`;
}
...
const Alerts = () => {
  ...
  useEffect(() => {
    const initial: AlertEntry[] = [
      { id: ++idRef.current, timestamp: new Date(Date.now() - 300000).toLocaleString(), zone: 'Silk Board Junction', status: 'violation', message: 'Encroachment detected in No-Parking Zone', challanNo: generateChallan(), vehicleType: randomVehicle(), bbox: randomBbox() },
      { id: ++idRef.current, timestamp: new Date(Date.now() - 240000).toLocaleString(), zone: 'Silk Board Junction', status: 'clear', message: 'Zone cleared — no violations', challanNo: '—', vehicleType: 'car', bbox: randomBbox() },
      { id: ++idRef.current, timestamp: new Date(Date.now() - 180000).toLocaleString(), zone: 'KR Market', status: 'violation', message: 'Encroachment detected in No-Parking Zone', challanNo: generateChallan(), vehicleType: randomVehicle(), bbox: randomBbox() },
      { id: ++idRef.current, timestamp: new Date(Date.now() - 60000).toLocaleString(), zone: 'MG Road', status: 'violation', message: 'Encroachment detected in No-Parking Zone', challanNo: generateChallan(), vehicleType: randomVehicle(), bbox: randomBbox() },
      { id: ++idRef.current, timestamp: new Date().toLocaleString(), zone: 'MG Road', status: 'clear', message: 'Zone cleared — no violations', challanNo: '—', vehicleType: 'car', bbox: randomBbox() },
    ];
    setAlerts(initial);
  }, []);
